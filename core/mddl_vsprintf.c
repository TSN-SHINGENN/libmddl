 ...
 * @return 指定可変長引数のポインタ
 **/
static void *get_va_pointer(va_list *const ap_p)
{
...
/**
 * @fn static int _own_vsnprintf(const multios_xtoa_output_method_t *const method_p, const char *const fmt, va_list *const ap_p)
 ...
 * @retval -1 失敗
 **/
static int _own_vsnprintf(const xtoa_output_method_t *const method_p, const char *const fmt, va_list *const ap_p)
{
    int result;
    int lenofneeders = 0;
    const char *fptr;
...
//	    xil_printf("%s : pre strings fini." EOL_CRLF, __func__);
	...
	if(*fptr == '*') {
	    length = get_signed( ap_p, _IS_Normal);
	    ++fptr;
	} else {
        ...
        if (*fptr == '.') {
            ...
            if (*fptr == '*') {
		++fptr;
		precision = get_signed( ap_p, _IS_Normal);
	    } else {
		while (isdigit(*fptr) ) {
		    precision *= 10;
	...
	if(1) {
	    ...
	    case 'd':
	    case 'i': {
		const long long int lli = get_signed( ap_p, int_type);
...
//		xil_printf("%s : d i lenofneeders=%d retlen=%d" EOL_CRLF, __func__, lenofneeders, retlen); 
	    ...
	    case 'u': {
//		const int remain = (int)(max_len - lenofneeders);
		const unsigned long long int ulli = get_unsigned( ap_p, int_type);
...
//		specs.f.alternative = 1;
	    ...
	    case 'X': {
//		const int remain = (int)(max_len - lenofneeders);
		const unsigned long long int ulli = (int_type == _IS_Ptr) ? (unsigned long long)get_va_pointer( ap_p) : get_unsigned( ap_p, int_type);
//		multios_xtoa_output_method_t m = set_area_method( *method_p, bufp, remain);
		int retlen = 0;
...
#endif /* end of USE_FLOAT_FORMAT */
	    case 's': {
		const char *s = (char*)get_va_pointer(ap_p);
		size_t tmp;
