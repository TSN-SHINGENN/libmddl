...
void _mddl_mallocater_dump_region_list(mddl_mallocater_t const *const self_p)
{
    ...
    const char * const dfmt = "%04d:p=0x%p(buf_ptr:%p) magic_is=%s size=%s FLAG=%s next_ptr=%p" EOL_CRLF;
	...
	// dfmt = "%04d:p=0x%p(buf_ptr:%p) magic_is=%s size=%s FLAG=%s next_ptr=%p" EOL_CRLF;
	DMSG(  dfmt, n, p, (intptr_t)GET_HEAD2PTR(p), magic_is, p->size, flag, (uintptr_t)&(p->next_p));
    }
